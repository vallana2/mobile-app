...
export default function CreateListingScreen({ navigation }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Save photo URL locally using AsyncStorage
      if (form.photoUrl && res.data.id) {
        const stored = await AsyncStorage.getItem('photoUrls');
        const photoUrls = stored ? JSON.parse(stored) : {};
        photoUrls[res.data.id] = form.photoUrl;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TextInput style={styles.input} placeholder="e.g. Cozy Apartment in Kigali"
      ...
      <TextInput style={styles.input} placeholder="e.g. Kigali, Rwanda"
        value={form.location} onChangeText={v => setForm({ ...form, location: v })}
        placeholderTextColor="#aaa" />

      <Text style={styles.label}>Price per night ($)</Text>
      <TextInput style={styles.input} placeholder="e.g. 80"
        value={form.pricePerNight} onChangeText={v => setForm({ ...form, pricePerNight: v })}
      ...
      <TextInput style={styles.input} placeholder="wifi, kitchen, parking"
        value={form.amenities} onChangeText={v => setForm({ ...form, amenities: v })}
        placeholderTextColor="#aaa" />

      <Text style={styles.label}>📷 Photo URL</Text>
      <TextInput style={styles.input}
        placeholder="https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=800"
        value={form.photoUrl} onChangeText={v => setForm({ ...form, photoUrl: v })}
        placeholderTextColor="#aaa" />

      {form.photoUrl ? (
        <View style={styles.previewBox}>
          <Text style={styles.previewLabel}>Photo Preview:</Text>
          <Image source={{ uri: form.photoUrl }} style={styles.preview} resizeMode="cover" />
        </View>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: '#ddd', borderRadius: 12, padding: 16, marginBottom: 20, fontSize: 16, color: '#222' },
  typeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 20 },
  typeBtn: { borderWidth: 1, borderColor: '#ddd', borderRadius: 8, padding: 8, paddingHorizontal: 12 },
  ...
  button: { backgroundColor: '#FF385C', borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 8, marginBottom: 40 },
